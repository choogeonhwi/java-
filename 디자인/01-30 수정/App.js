    ...
    try {
      const res = await axios.get("/api/board", {
        params: {
          ...
          BOARD_IMG_NAME1: imageName1, // 깔끔하게 파일명만 전달
          BOARD_IMG_BYTE: imageSize, // 파일 사이즈 추가
          // TEST: imageName, // 테스트 코드
        },
...
const handleImageChange = (e) => {
  ...
  reader.onload = (event) => {
    ...
    // 파일 저장
    saveImageToFile(blob, imagepath);
    
    // 이미지 파일 이름을 즉시 사용하기 위해 handleSubmit 함수 호출
    handleWriteSubmit(filenameParts[0]);
  };
  ...
  const saveImageToFile = (blob, path) => {
    ...
    link.href = url;
    link.download = `${imageName1}`; // 파일명 설정
    link.click();
